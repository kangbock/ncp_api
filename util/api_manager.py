...
class APIManager:
    ...
    def api_manager(self, data):
        ...
        logging.info(f"NCP Monthly Report v2 - Start Time: {str_start_time}, End Time: {str_end_time}")
        ...
        excluded_functions = {
            # 특정 고객사 리소스 제외
            "CUSTOMER_ID": [
                "api.postgresql_list.PostgreSQL.postgresql_list", 
                "api.mysql_list.MySQL.mysql_list", 
                "api.mssql_list.MSSQL.mssql_list", 
                "api.mongodb_list.MongoDB.mongodb_list", 
                "api.redis_list.Redis.redis_list", 
                "api.clouddb_list.CloudDB.clouddb_list",
                "api.nas_list.NAS.classic_nas_list",
                "api.nas_metrics.NASMetrics.classic_nas_metrics",
        ...
        included_functions = {
            "CUSTOMER_ID_20TH": [
                "api.server_metrics.CloudInsight.classic_server_metrics",
        ...
        for row in data:
            ...
            # 제외할 고객사와 리소스 필터링
            excluded_patterns = excluded_functions.get(customer_id, [])
            filtered_functions = filter(
                lambda func: not any(re.match(pattern, f"{func.__module__}.{func.__qualname__}") for pattern in excluded_patterns),
                api_functions
            )

#            # 실행할 특정 고객사와 리소스 필터링 (파이프라인에서 주석처리 제거하여 활성화)
#            included_patterns = included_functions.get(customer_id, [])
#            filtered_functions = filter(
#                lambda func: any(re.match(pattern, f"{func.__module__}.{func.__qualname__}") for pattern in included_patterns),
#                api_functions
#            )
