...
class PublicIP(init):
    ...
    def classic_public_ip_list(self):
        """
        공인 ip 목록을 조회하는 메서드 <br>
        return: 공인 ip 목록 (딕셔너리 형식) <br>
        ...
        uri = "/server/v2/getPublicIpInstanceList"
        ...
        try:
            ...
            if not len(data['getPublicIpInstanceListResponse']['publicIpInstanceList']) == 0:
                logging.info(f"{self.customer_id} - {Platform_type} Public IP Total Number : "  + str(len(data['getPublicIpInstanceListResponse']['publicIpInstanceList'])))

                for public_ip in data['getPublicIpInstanceListResponse']['publicIpInstanceList']:
                    public_ip['customer_id'] = self.customer_id
                    public_ip["datetime"] = self.datetime
                    public_ip["publicIpInstanceStatus"] = public_ip["publicIpInstanceStatus"]["code"]
                    logging.info(f"{self.customer_id} - {Platform_type} Public IP : " + public_ip['publicIp'])
                    result.append(public_ip)

                # JSON 파일 경로 생성
                json_file_path = f"./json/{self.customer_id}/{self.customer_type}_{Platform_type}_public_ip_list.json"
                JSONUtil.save_to_json(result, json_file_path)

                # DataFrame으로 변환 및 Database에 저장
                columns = ["customer_id", "datetime", "publicIp", "publicIpInstanceStatus"]
                df = pd.DataFrame(result, columns=columns)
                df["datetime"] = pd.to_datetime(df["datetime"], errors='coerce')
                table_name = f"{self.customer_type}_{Platform_type}_public_ip_list"
                DatabaseUtil().save_to_db(df, table_name)

        except Exception as e:
            logging.error(f"{self.customer_id} - Error fetching public ip list: {e}")
            # 웹훅 body 생성
            body = [
                {
                    "type": "TextBlock",
                ...
                },
                {
                    ...
                    "facts": [
                        ...
                        {"title": "Service Type:", "value": "Public IP"},
    ...
    def vpc_public_ip_list(self):
        ...
        uri = "/vserver/v2/getPublicIpInstanceList"
