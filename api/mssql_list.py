...
class MSSQL(init):
    ...
    def mssql_list(self):
        ...
        result = []
        raw_data = []
        uri = "/vmssql/v2/getCloudMssqlInstanceList"

        try:
            for regionCode in region:
                uri_with_params = uri + f"?responseFormatType=json&regionCode={regionCode}"
                ...
                if not len(data["getCloudMssqlInstanceListResponse"]["cloudMssqlInstanceList"]) == 0:
                    logging.info(f"{self.customer_id} - VPC MSSQL Service {regionCode} Region Total Number : "  + str(len(data["getCloudMssqlInstanceListResponse"]["cloudMssqlInstanceList"])))

                    for mssql in data["getCloudMssqlInstanceListResponse"]["cloudMssqlInstanceList"]:
                        for mssqlserver in mssql["cloudMssqlServerInstanceList"]:
                            mssql_data = {
                                "customer_id": self.customer_id,
                                "datetime": self.datetime,
                                "serviceName": mssql["cloudMssqlServiceName"],
                                "serverName": mssqlserver["cloudMssqlServerName"],
                                "status": mssqlserver["cloudMssqlServerInstanceStatusName"],
                                "vpcName": VPC.vpc_list(self, mssqlserver["vpcNo"], regionCode),
                                "regionCode": regionCode,
                                "cpuCount": mssqlserver.get("cpuCount", 0),
                                "memorySize": mssqlserver["memorySize"] // (1024**3),
                                "dataStorageSize": mssqlserver["dataStorageSize"] // (1024**3),
                            }
                            result.append(mssql_data)
                            logging.info(f"{self.customer_id} - VPC MSSQL Service {regionCode} Region Server : " + mssqlserver["cloudMssqlServerName"])
                        raw_data.append(mssql)

                    # JSON 파일 경로 생성
                    json_file_path = f"./json/{self.customer_id}/{self.customer_type}_VPC_mssql_list.json"
                    ...
                    columns = ["customer_id", "datetime", "serviceName", "serverName", "status", "vpcName", "regionCode", "cpuCount", "memorySize", "dataStorageSize"]
                    ...
                    table_name = f"{self.customer_type}_VPC_mssql_list"
                    DatabaseUtil().save_to_db(df, table_name)

        except Exception as e:
            logging.error(f"{self.customer_id} - Error processing region {region}: {e}")
            # 웹훅 body 생성
            body = [
                {
                    "type": "TextBlock",
                ...
                },
                {
                    ...
                    "facts": [
                        ...
                        {"title": "Service Type:", "value": "MSSQL"},
