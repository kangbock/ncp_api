...
class MySQL(init):
    ...
    def mysql_list(self):
        ...
        result = []
        raw_data = []
        uri = "/vmysql/v2/getCloudMysqlInstanceList"
        ...
        try:
            ...
            if not len(data["getCloudMysqlInstanceListResponse"]["cloudMysqlInstanceList"]) == 0:
                logging.info(f"{self.customer_id} - VPC MySQL Service Total Number : "  + str(len(data["getCloudMysqlInstanceListResponse"]["cloudMysqlInstanceList"])))

                for mysql in data["getCloudMysqlInstanceListResponse"]["cloudMysqlInstanceList"]:
                    for mysqlserver in mysql["cloudMysqlServerInstanceList"]:
                        mysql_data = {
                            "customer_id": self.customer_id,
                            "datetime": self.datetime,
                            "serviceName": mysql["cloudMysqlServiceName"],
                            "serverName": mysqlserver["cloudMysqlServerName"],
                            "serverRole": mysqlserver["cloudMysqlServerRole"]["codeName"],
                            "status": mysqlserver["cloudMysqlServerInstanceStatusName"],
                            "vpcName": VPC.vpc_list(self, mysqlserver["vpcNo"], mysqlserver["regionCode"]),
                            "zoneCode": mysqlserver.get("zoneCode", "Unknown"),
                            "cpuCount": mysqlserver.get("cpuCount", 0),
                            "memorySize": mysqlserver["memorySize"] // (1024**3),
                            "dataStorageSize": mysqlserver["dataStorageSize"] // (1024**3),
                            "cloudMysqlServerInstanceNo": mysqlserver["cloudMysqlServerInstanceNo"],
                        }
                        result.append(mysql_data)                    
                        logging.info(f"{self.customer_id} - VPC MySQL : " + mysqlserver["cloudMysqlServerName"])
                    raw_data.append(mysql)

                # JSON 파일 경로 생성
                json_file_path = f"./json/{self.customer_id}/{self.customer_type}_VPC_mysql_list.json"
                ...
                table_name = f"{self.customer_type}_VPC_mysql_list"
                DatabaseUtil().save_to_db(df, table_name)

        except Exception as e:
            logging.error(f"{self.customer_id} - Error fetching mysql list: {e}")
            # 웹훅 body 생성
            body = [
                {
                    "type": "TextBlock",
                ...
                },
                {
                    ...
                    "facts": [
                        ...
                        {"title": "Service Type:", "value": "MySQL"},
